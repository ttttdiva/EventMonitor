...
```bash
# 全ての欠落ファイルを再処理
python scripts/fix/reprocess_missing_files.py

# ドライランで確認のみ
python scripts/fix/reprocess_missing_files.py --dry-run

# 最初の10件のみ処理
python scripts/fix/reprocess_missing_files.py --limit 10

# 動画ファイルのみ処理
python scripts/fix/reprocess_missing_files.py --file-type videos
```
...
## 4. HuggingFaceリポジトリの履歴管理

### コミット履歴の圧縮（super_squash_history）

HuggingFaceのデータセットリポジトリの過去のコミット履歴を圧縮して、ストレージを節約する方法：
...
- **リポジトリは削除されません**: データとリポジトリ自体は保持されます
- **過去のコミットが圧縮されます**: 複数のコミットが1つにまとまります
- **ストレージ節約**: 不要な履歴が削除されてストレージが解放されます
- **元に戻せません**: 一度実行すると履歴の復元はできません

### 使用タイミング

- リポジトリのサイズが大きくなりすぎた時
- 過去の細かいコミット履歴が不要になった時
- HuggingFaceのストレージ制限に近づいた時
