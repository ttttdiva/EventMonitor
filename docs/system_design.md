...
### 1. 監視アカウント（通常アカウント）
**account_type: normal** (notification=noticeで通知あり、空欄=通知なし)

#### 処理フロー
1. TwitterからツイートをAPI/twscrapeで取得
2. gallery-dlでメディアをダウンロード（設定されたメディア保存ディレクトリ）
...
### メディアファイルの保存先
- **監視アカウント**: `config.yaml`の`media_storage`で設定されたディレクトリ
  - デフォルト: `images/`, `videos/` （プロジェクトルート相対）
  - 設定例: `/mnt/f/48_EventMonitor_log/images`, `/mnt/f/48_EventMonitor_log/videos`
  - ユーザー名ごとのサブディレクトリ
  - 永続的に保持
