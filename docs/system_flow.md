...
3. **メディアダウンロード**
   - 新規ツイートの画像を設定されたメディア保存パス`{images_path}/{username}/`にダウンロード
   - 動画を設定されたメディア保存パス`{videos_path}/{username}/`にダウンロード
...
- アカウントごとに設定されたメディア保存パス配下の各ユーザーディレクトリを一括アップロード
...
2. **段階的取得段階**
   ```
   新着ありの場合:
   ├─ フェーズ1: twscrapeで新規ツイートを最大20件取得
   ├─ フェーズ2: まだ新規が続くなら追加で20件取得（合計40件）
   ├─ フェーズ3: それでも不足するなら、制限なしで残りを取得し追加分があればgallery-dlでJSON全取得
   ├─ 各フェーズ開始時に既知ID（固定ツイなど）へ当たったら1度だけスキップしてから判定
   └─ 40件以内で収まった場合はgallery-dlをスキップ
   ```
...
# 段階取得の例（概念）
CHUNK_SIZE = 20
chunk = fetch_twscrape(limit=CHUNK_SIZE)
if len(chunk) == CHUNK_SIZE:
    chunk += fetch_twscrape(limit=CHUNK_SIZE, skip_ids=chunk.ids)
    if len(chunk) >= CHUNK_SIZE * 2:
        chunk += fetch_twscrape(limit=None, skip_ids=chunk.ids)
        trigger_gallery_dl()
else:
    skip_gallery_dl()
```
