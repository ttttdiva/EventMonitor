...
if %errorlevel% neq 0 (
    echo [警告] ffmpeg が見つかりません。
    echo 動画の保存機能を使用する場合は、ffmpegをインストールしてPATHに通してください。
) else (
...
if %errorlevel% neq 0 (
    echo [警告] rclone が見つかりません。
    echo バックアップ機能を使用する場合は、rcloneをインストールしてPATHに通してください。
) else (
    echo rclone が見つかりました
)
