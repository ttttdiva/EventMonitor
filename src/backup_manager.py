...
from .path_utils import to_absolute_path, get_media_base_paths
...
class BackupManager:
    ...
    def __init__(self, config: dict, db_manager=None):
        ...
        try:
            ...
            # upload_large_folder用にグローバル認証も設定
    ...
    def _extract_base_repo_name(self, repo_name: str) -> str:
        ...
        # 例: "disguisequence/EventMonitor_1" → "disguisequence/EventMonitor"
    ...
    def _handle_upload_error(self, error: Exception) -> bool:
        ...
        if "timed out" in error_msg.lower() or "timeout" in error_msg.lower():
            self.logger.warning(f"Timeout error, retrying after 10s...")
            time.sleep(10)
            return True

        # 接続エラー（一時的なネットワーク障害）
        if any(kw in error_msg.lower() for kw in ["connection", "refused", "reset", "broken pipe"]):
            self.logger.warning(f"Connection error, retrying after 10s...")
            time.sleep(10)
            return True
    ...
    def _ensure_repo_exists(self):
        ...
        try:
            ...
            # 404 Not Foundエラー（リポジトリが存在しない）の場合のみ作成を試行
            if "404" in str(e) or "Not Found" in str(e):
                ...
                try:
                    ...
                    # 409 Conflictエラー（既に存在）の場合は正常とみなす
                    if "409" in str(create_error) or "already" in str(create_error).lower():
                        self.logger.info(f"Repository {self.full_repo_name} already exists")
                        return
                    else:
                ...
                # その他のエラーの場合は再度発生させる
                self.logger.error(f"Failed to check repository existence: {e}")
                raise
    
    async def backup_and_cleanup_works(
        self,
        works: List[Dict[str, Any]],
        platform: str,
        user_id: str,
    ) -> Dict[str, List[str]]:
        """任意プラットフォームの作品メディアをHFにアップロードし、成功後にローカルファイルを削除

        DB保存済みの作品に対して呼ぶこと（save_{platform}_works() の後）。
        huggingface_backup.enabled が false の場合は何もしない。

        Args:
            works: 作品dictのリスト。各dictに 'id' と 'local_media'（相対パスリスト）が必要
            platform: プラットフォーム名 (e.g. 'pixiv', 'fanbox')
            user_id: ユーザー/クリエイターID

        Returns:
            Dict[work_id -> HF URLリスト]（成功した作品のみ）
        """
        ...
        try:
            ...
            self.logger.error(f"Failed to ensure HF repo exists: {e}")
            return {}
        ...
        for work in works:
            ...
            for media_path in local_media:
                media_file = to_absolute_path(media_path, self.config)
                if not media_file.exists():
                    self.logger.warning(f"File not found for {platform} work {work_id}: {media_file}")
                    continue

                media_type = 'video' if 'videos/' in str(media_path).replace('\\', '/') else 'image'
                hf_url = await self._upload_file_with_retry(media_file, media_type)
                if hf_url:
                    hf_urls.append(hf_url)
                else:
                    self.logger.error(f"Failed to upload {media_file} for {platform} work {work_id}")
                    all_uploaded = False
                    break

            if not all_uploaded or not hf_urls:
                continue

            # DB更新
            if self.db_manager:
                try:
                    self.db_manager.update_work_hf_urls(
                        platform=platform,
                        work_id=work_id,
                        huggingface_urls=hf_urls,
                    )
                except Exception as e:
                    self.logger.error(f"Failed to update HF URLs in DB for {platform} work {work_id}: {e}")
                    continue
            ...
            for media_path in local_media:
                try:
                    ...
                    if media_file.exists():
                        media_file.unlink()
                        self.logger.debug(f"Deleted local file after HF upload: {media_file}")
                except Exception as e:
                    self.logger.warning(f"Failed to delete local file {media_path}: {e}")

            results[work_id] = hf_urls

        uploaded_count = sum(len(urls) for urls in results.values())
        if results:
            self.logger.info(
                f"HF backup for {platform} user {user_id}: "
                f"{len(results)}/{len(works)} works uploaded ({uploaded_count} files)"
            )
    ...
    async def backup_tweet_and_save(self, tweet: Dict[str, Any], username: str, is_log_only: bool = False, hydrus_client=None, is_first_run: bool = False) -> bool:
        ...
        try:
            ...
            # アップロード成功後、ローカルファイルを削除（HFにバックアップ済み）
            if tweet.get('local_media') and hf_urls:
                for media_path in tweet['local_media']:
                    try:
                        media_file = to_absolute_path(media_path, self.config)
                        if media_file.exists():
    ...
    async def backup_tweets(self, new_tweets: List[Dict[str, Any]], is_log_only: bool = False):
        ...
        try:
            ...
            for tweet in new_tweets:
                ...
                if is_log_only:
                    # logアカウントの場合、media_urlsからファイルパスを構築
                    if tweet.get('media_urls'):
                        ...
                        tweet_id = tweet.get('id')
                        if username and media_urls:
                            # 既存の_construct_local_media_pathsメソッドを使用
                            media_paths = self._construct_local_media_paths(username, media_urls, tweet_id)
                else:
                    # 監視アカウントの場合、local_mediaを使用
                    if tweet.get('local_media'):
                        media_paths = tweet['local_media']
    ...
    # プラットフォーム別フォルダ名プレフィックス（extractorの実装に基づく）
    PLATFORM_FOLDER_PREFIX = {
        'twitter': '',       # folder = username
        'pixiv': '',         # folder = user_id (数値)
        'fanbox': 'fanbox_', # folder = fanbox_{creator_id}
        'kemono': None,      # 特殊: {service}_{user_id}
        'fantia': 'fantia_',
        'nijie': 'nijie_',
        'skeb': 'skeb_',
        'misskey': 'misskey_',
        'bluesky': 'bluesky_',
        'gelbooru': 'gelbooru_',
        'poipiku': '',       # folder = user_id
        'tinami': '',        # folder = prof_id
        'privatter': '',     # folder = user_id
    }

    async def generate_and_upload_creator_mapping(self):
        ...
        if not self.db_manager or not self.db_manager.Session:
            self.logger.warning("データベースが利用できないため、クリエイターマッピング生成をスキップ")
            return

        try:
            from .database import DatabaseManager, KemonoWork, KemonoLogOnlyWork
            ...
            try:
                for platform, (monitor_model, log_model) in DatabaseManager.PLATFORM_MODELS.items():
                    identity_field = self.db_manager._get_platform_identity_field(platform)
                    prefix = self.PLATFORM_FOLDER_PREFIX.get(platform, '')
            ...
            try:
                ...
                self.logger.info(f"creator_mapping.json をアップロード完了（{len(mapping)}件, {file_size:.1f} KB）")
            finally:
                temp_file.unlink(missing_ok=True)

        except Exception as e:
            self.logger.error(f"クリエイターマッピング生成に失敗: {e}", exc_info=True)
    ...
    def _enrich_creator_mapping(self, mapping: dict):
        ...
        db_path = self.db_manager.config.get('database', {}).get('path', 'data/eventmonitor.db')
        ...
        try:
            # Twitter (all_tweets + log_only_tweets)
                ...
                try:
                    ...
                    for username, file_count, sensitive_count in cursor.fetchall():
                        if username in mapping:
                            mapping[username].setdefault("file_count", 0)
                            mapping[username]["file_count"] += file_count
                            mapping[username].setdefault("sensitive_count", 0)
                            mapping[username]["sensitive_count"] += (sensitive_count or 0)
                except Exception:
                    pass
            ...
            for platform, (table, id_field, prefix) in platform_configs.items():
                try:
                    ...
                    cursor.execute(f"SELECT {id_field}, COUNT(*) {sensitive_sel} FROM [{table}] GROUP BY {id_field}")
                    ...
                    if has_tags:
                        cursor.execute(f"SELECT {id_field}, tags FROM [{table}] WHERE tags IS NOT NULL AND tags != '' AND tags != '[]'")
                        ...
                        for folder, counter in user_tags.items():
                            if folder in mapping:
                                mapping[folder]["tags"] = [tag for tag, _ in counter.most_common(30)]
                except Exception:
                    continue
            ...
            for table in ['kemono_works', 'kemono_log_only_works']:
                try:
                    ...
                    has_sensitive = 'sensitive' in columns
                    sensitive_sel = ", SUM(CASE WHEN sensitive = 1 THEN 1 ELSE 0 END)" if has_sensitive else ""
                    cursor.execute(f"SELECT user_id, service, COUNT(*) {sensitive_sel} FROM [{table}] GROUP BY user_id, service")
                    for row in cursor.fetchall():
                        uid, service, count = row[0], row[1], row[2]
            ...
            self.logger.error(f"マッピング集約に失敗: {e}", exc_info=True)
        finally:
            conn.close()
    ...
    async def upload_remaining_media(self, hydrus_client=None):
        ...
        """
        # HuggingFaceバックアップとlogアカウントの両方が無効な場合は何も実行しない
        ...
        if not backup_enabled and not log_accounts_enabled:
            self.logger.info("HuggingFace backup and log accounts upload are both disabled, skipping upload_remaining_media")
            return

        if backup_enabled:
            self.logger.info("HuggingFace backup is enabled - processing unprocessed media")
        if log_accounts_enabled:
            self.logger.info("Log accounts upload is enabled - processing unprocessed log media")

        try:
            self.logger.info("Starting upload of remaining media to HuggingFace...")

            # まずデータベースからアカウント別の未アップロード統計を取得
            monitoring_account_stats = {}
            log_account_stats = {}

            if self.db_manager:
                # 監視アカウントの統計
                if backup_enabled:
                    monitoring_account_stats = await self._get_unuploaded_stats_from_db('monitoring')

                # logアカウントの統計
                if log_accounts_enabled:
                    log_account_stats = await self._get_unuploaded_stats_from_db('log')

            # バッチ処理対象と個別処理対象を分ける
            monitoring_batch_accounts = []
            monitoring_individual_accounts = []
            log_batch_accounts = []
            log_individual_accounts = []

            # 監視アカウントの振り分け
            for username, stats in monitoring_account_stats.items():
                if stats['unuploaded_count'] >= 15:
                    monitoring_batch_accounts.append(username)
                    self.logger.info(f"Monitoring account {username}: {stats['unuploaded_count']} files -> batch upload")
                elif stats['unuploaded_count'] > 0:
                    monitoring_individual_accounts.append(username)
                    self.logger.info(f"Monitoring account {username}: {stats['unuploaded_count']} files -> individual upload")

            # logアカウントの振り分け
            for username, stats in log_account_stats.items():
                if stats['unuploaded_count'] >= 15:
                    log_batch_accounts.append(username)
                    self.logger.info(f"Log account {username}: {stats['unuploaded_count']} files -> batch upload")
                elif stats['unuploaded_count'] > 0:
                    log_individual_accounts.append(username)
                    self.logger.info(f"Log account {username}: {stats['unuploaded_count']} files -> individual upload")

            # 1. 監視アカウントのバッチアップロード（ファイルが存在するもののみ）
            for username in monitoring_batch_accounts:
                # ディレクトリの存在を事前チェック（絶対パスで確認）
                images_base, videos_base = get_media_base_paths(self.config)
                images_dir = images_base / username
                videos_dir = videos_base / username
                if images_dir.exists() or videos_dir.exists():
                    await self.batch_upload_folder(
                        folder_path=images_base.parent,  # /mnt/f/48_EventMonitor_log
                        account_type='monitoring',
                        encrypt=self.rclone_client is not None,
                        delete_after=False,
                        username=username
                    )
                else:
                    self.logger.warning(f"Skipping batch upload for {username} - no local files exist")
                    # ファイルがない場合は個別処理に追加
                    monitoring_individual_accounts.append(username)

            # 2. logアカウントのバッチアップロード（ファイルが存在するもののみ）
            for username in log_batch_accounts:
                # ディレクトリの存在を事前チェック（絶対パスで確認）
                images_base, videos_base = get_media_base_paths(self.config)
                images_dir = images_base / username
                videos_dir = videos_base / username
            ...
            # 3. 監視アカウントの個別処理
            if monitoring_individual_accounts:
                self.logger.info(f"Processing {len(monitoring_individual_accounts)} monitoring accounts individually")
                await self._process_accounts_individually(
                    monitoring_individual_accounts,
                    'monitoring',
            ...
            # 4. logアカウントの個別処理
            if log_individual_accounts:
                self.logger.info(f"Processing {len(log_individual_accounts)} log accounts individually")
                await self._process_accounts_individually(
                    log_individual_accounts,
                    'log',
                    hydrus_client=None
            ...
            # 非Twitterプラットフォームの未アップロード分を処理
            if backup_enabled:
                await self._upload_remaining_platform_works()
    ...
    async def _upload_remaining_platform_works(self):
        """非Twitterプラットフォームの未アップロードメディアを処理"""
        import asyncio
        ...
        PLATFORM_TABLES = {
            ...
            'kemono': 'kemono_works',
            'tinami': 'tinami_works',
            'poipiku': 'poipiku_works',
            'fantia': 'fantia_works',
            'nijie': 'nijie_works',
            'skeb': 'skeb_works',
            'misskey': 'misskey_works',
            'gelbooru': 'gelbooru_works',
            'fanbox': 'fanbox_works',
        }
        ...
        try:
            ...
            self.logger.error(f"Failed to connect to DB for remaining platform works: {e}")
            return
        ...
        try:
            for platform, table_name in PLATFORM_TABLES.items():
                try:
                    cursor.execute(f'''
                        SELECT id, user_id, local_media
                        FROM {table_name}
                        WHERE local_media IS NOT NULL
                          AND local_media != ''
                          AND local_media != '[]'
                          AND (huggingface_urls IS NULL OR huggingface_urls = '' OR huggingface_urls = '[]')
                        LIMIT 500
                    ''')
                    ...
                    self.logger.info(f"Found {len(rows)} {platform} works with unuploaded media")

                    # ユーザーごとにグループ化
                    user_works: Dict[str, List[Dict[str, Any]]] = {}
                    for work_id, uid, local_media_json in rows:
                        local_media = json.loads(local_media_json) if local_media_json else []
                        if not local_media:
                            continue
                        user_works.setdefault(uid, []).append({
                            'id': work_id,
                            'local_media': local_media,
                        })

                    for uid, works in user_works.items():
                        await self.backup_and_cleanup_works(
                            works=works,
                            platform=platform,
                            user_id=uid,
                        )
                    ...
                    self.logger.error(f"Failed to process remaining {platform} works: {e}")
                    continue
        finally:
            conn.close()

    async def _upload_remaining_from_db(self, hydrus_client=None, backup_enabled=False, log_accounts_enabled=False):
        """データベースから未アップロード分を取得して処理

        Args:
            hydrus_client: HydrusClientインスタンス（オプション）
            backup_enabled: 監視アカウントのバックアップが有効か
            log_accounts_enabled: logアカウントのアップロードが有効か
        """
        try:
            ...
            all_tweets_query = session.query(AllTweets).filter(
                ...
                (AllTweets.huggingface_urls == '[]')
            ).order_by(AllTweets.tweet_date.asc())  # 古い順でソート

            unprocessed_all_tweets = all_tweets_query.all()

            # log_only_tweetsテーブルで media_urls があるが huggingface_urls がない行を取得
            from .database import LogOnlyTweet
            log_tweets_query = session.query(LogOnlyTweet).filter(
                ...
                (LogOnlyTweet.huggingface_urls == '[]')
            ).order_by(LogOnlyTweet.tweet_date.asc())  # 古い順でソート

            unprocessed_log_tweets = log_tweets_query.all()

            all_count = len(unprocessed_all_tweets)
            log_count = len(unprocessed_log_tweets)
            # 有効な機能に応じて処理対象を決定
            process_all_tweets = backup_enabled and all_count > 0
            process_log_tweets = log_accounts_enabled and log_count > 0

            if not process_all_tweets and not process_log_tweets:
                self.logger.info("No unprocessed media found or no enabled features")
                session.close()
                return

            self.logger.info(f"Found {all_count} all_tweets ({'will process' if process_all_tweets else 'skip'}) and {log_count} log_only_tweets ({'will process' if process_log_tweets else 'skip'})")
            
            processed_count = 0
            
            # all_tweetsの処理（監視アカウント設定が有効な場合のみ）
            if process_all_tweets:
                for tweet in unprocessed_all_tweets:
                    try:
                        ...
                        tweet_data = {
                            ...
                            'sensitive': bool(getattr(tweet, 'sensitive', False)),
                        }
            ...
            # log_only_tweetsの処理（logアカウント設定が有効な場合のみ）
            if process_log_tweets:
                for tweet in unprocessed_log_tweets:
                    try:
                        ...
                        # ローカルメディアパスを構築
                        local_media_paths = self._construct_local_media_paths(tweet.username, media_urls, tweet.id)
                        ...
                        # logアカウント用のバックアップ処理を実行
                        await self.backup_tweet_and_save(tweet_data, tweet.username, is_log_only=True)

                        processed_count += 1

                        if processed_count % 10 == 0:
                            self.logger.info(f"Processed {processed_count}/{log_count} log tweets")
            ...
            session.close()
            self.logger.info(f"Completed processing {processed_count} tweets from database (all_tweets: {all_count}, log_only_tweets: {log_count})")
            
        except Exception as e:
            self.logger.error(f"Failed to upload remaining from DB: {e}")
            raise

    async def _get_unuploaded_stats_from_db(self, account_type='monitoring'):
        """データベースからアカウント別の未アップロード統計を取得

        Args:
            account_type: 'monitoring' または 'log'

        Returns:
            dict: {username: {'unuploaded_count': n, 'tweet_ids': [...]}}
        """
        ...
        try:
            if account_type == 'monitoring':
                # all_tweetsテーブルから未アップロード分を取得
                ...
                for tweet in tweets:
                    ...
                    if username not in account_stats:
                        account_stats[username] = {'unuploaded_count': 0, 'tweet_ids': []}

                    # メディアファイル数をカウント
                    media_urls = json.loads(tweet.media_urls) if tweet.media_urls else []
                    account_stats[username]['unuploaded_count'] += len(media_urls)
                    account_stats[username]['tweet_ids'].append(tweet.id)

            else:  # log
                # log_only_tweetsテーブルから未アップロード分を取得
                query = session.query(LogOnlyTweet).filter(
                    LogOnlyTweet.media_urls.isnot(None),
                    LogOnlyTweet.media_urls != '',
                    LogOnlyTweet.media_urls != '[]'
                ).filter(
    ...
    async def _process_accounts_individually(self, accounts, account_type, hydrus_client=None):
        """指定されたアカウントのツイートを個別処理
        ...
        try:
            for username in accounts:
                self.logger.info(f"Processing {account_type} account {username} individually")

                if account_type == 'monitoring':
                    # all_tweetsテーブルから該当アカウントの未アップロード分を取得
                    ...
                    # log_only_tweetsテーブルから該当アカウントの未アップロード分を取得
                    tweets = session.query(LogOnlyTweet).filter(
                        ...
                        (LogOnlyTweet.huggingface_urls == '') |
                        (LogOnlyTweet.huggingface_urls == '[]')
                    ).order_by(LogOnlyTweet.tweet_date.asc()).all()
                ...
                for idx, tweet in enumerate(tweets):
                    try:
                        ...
                        if account_type == 'monitoring':
                            ...
                            # 監視アカウントはbackup_tweetsを使用
                            await self.backup_tweets([tweet_data])
                        else:
                            # logアカウントの場合はローカルメディアパスを構築
                            local_media_paths = self._construct_local_media_paths(tweet.username, media_urls, tweet.id)
                            ...
                            # logアカウントはbackup_tweet_and_saveを使用
                            await self.backup_tweet_and_save(tweet_data, username, is_log_only=True)

                        if (idx + 1) % 10 == 0:
                            self.logger.info(f"Processed {idx + 1}/{len(tweets)} tweets for {username}")

                        # レート制限対策
                        await asyncio.sleep(0.1)

                    except Exception as e:
                        self.logger.error(f"Failed to process tweet {tweet.id}: {e}")
                        continue

                self.logger.info(f"Completed processing {len(tweets)} tweets for {username}")
    ...
    def _construct_local_media_paths(self, username: str, media_urls: List[str], tweet_id: str = None) -> List[str]:
        """media_urlsからローカルファイルパスを推測・構築する

        Args:
            username: ユーザー名
            media_urls: 元のメディアURL一覧
            tweet_id: ツイートID（gallery-dlのファイル名形式に使用）

        Returns:
            List[str]: 存在するローカルファイルパス一覧
        """
        local_paths = []
        ...
        for i, media_url in enumerate(media_urls):
            try:
                # gallery-dlの命名規則: {tweet_id}_{index}.{ext}
                if tweet_id:
                    # メディアURLから拡張子を推測
                    if 'video' in media_url or '.mp4' in media_url:
                        possible_exts = ['mp4', 'mov', 'avi']
                        base_dir = videos_base
                    else:
                        possible_exts = ['jpg', 'png', 'gif', 'webp']
                    ...
                    for ext in possible_exts:
                        ...
                        # 拡張子が見つからない場合はフォールバックへ
                        continue

                # フォールバック: URLから直接ファイル名を推測
                ...
                if len(url_parts) > 0:
                    ...
                    # 可能なパスを試行
                    possible_paths = [
                        images_base / username / filename,
                        videos_base / username / filename,
                        images_base / username / f"{filename.rsplit('.', 1)[0]}.jpg",
                        images_base / username / f"{filename.rsplit('.', 1)[0]}.png",
                        videos_base / username / f"{filename.rsplit('.', 1)[0]}.mp4",
                    ]
                    ...
                    for path in possible_paths:
                        ...
                        self.logger.debug(f"Local file not found for media URL: {media_url}")

            except Exception as e:
                self.logger.warning(f"Failed to construct local path for {media_url}: {e}")
                continue
    ...
    def _calculate_unuploaded_stats(self, folder_path: Path, username: str, account_type: str = 'monitoring') -> tuple[int, int]:
        """未アップロード統計を計算
        ...
        Returns:
            tuple[int, int]: (未アップロードファイル数, 総ファイル数)
        """
        try:
            import pysqlite3 as sqlite3
            ...
            # 適切なテーブルを選択
            table_name = 'log_only_tweets' if account_type == 'log' else 'all_tweets'
            
            for media_dir in [account_images_dir, account_videos_dir]:
                if not media_dir.exists():
                    continue
                    
                # 対象ファイルパターン
                if 'images' in str(media_dir):
                ...
                for pattern in patterns:
                    for file_path in media_dir.glob(pattern):
                        ...
                        if result and result[0]:
                            try:
                                urls = json.loads(result[0])
                                # 実際にURLが含まれているかチェック（空のリストは未アップロード）
                                is_uploaded = bool(urls and any(url.strip() for url in urls if isinstance(url, str)))
                            except (json.JSONDecodeError, TypeError):
                                is_uploaded = False
            ...
            unuploaded_files = total_files - uploaded_files
            unuploaded_ratio = unuploaded_files / total_files if total_files > 0 else 0.0
            
            self.logger.info(f"Upload stats for {username}: {unuploaded_files} unuploaded files out of {total_files} total ({unuploaded_ratio:.1%})")
            return unuploaded_files, total_files
            
        except Exception as e:
            self.logger.error(f"Failed to calculate unuploaded stats: {e}")
            return 0, 0
    ...
    async def _batch_upload_plain_folder(self, folder_path: Path, username: str = None, 
        ...
        """暗号化なしでフォルダを一括アップロード（upload_large_folder/upload_file使用）"""
        if not username:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix=f"batch_upload_{username}_") as temp_dir:
                ...
                # 未アップロード統計を計算
                unuploaded_files, total_files = self._calculate_unuploaded_stats(folder_path, username, account_type)
                
                # 15ファイル以上が未アップロードならupload_large_folder、それ以外はupload_file
                max_retries = 3
                for attempt in range(max_retries):
                    try:
                        if unuploaded_files >= 15 and HAS_UPLOAD_LARGE_FOLDER:  # 15ファイル以上が未アップロードかつ関数が利用可能
                            self.logger.info(f"Using upload_large_folder for {file_count} files ({unuploaded_files} unuploaded files)")
                            upload_large_folder(
                            ...
                            if unuploaded_files >= 15 and not HAS_UPLOAD_LARGE_FOLDER:
                                self.logger.warning(f"Many unuploaded files ({unuploaded_files}) detected but upload_large_folder not available. Using individual upload_file.")
                            self.logger.info(f"Using individual upload_file for {file_count} files ({unuploaded_files} unuploaded files)")

                            # 個別ファイルアップロード
                            for file_path in temp_path.rglob('*'):
                                if file_path.is_file() and not any(pattern in str(file_path) for pattern in ["*.tmp", "*.temp", ".DS_Store", "Thumbs.db"]):
    ...
    async def _batch_upload_encrypted_folder(self, folder_path: Path, username: str = None,
        ...
        """フォルダを暗号化してアップロード（upload_large_folder/upload_file使用）"""
            ...
            for attempt in range(max_retries):
                try:
                    if unuploaded_files >= 15 and HAS_UPLOAD_LARGE_FOLDER:  # 15ファイル以上が未アップロードかつ関数が利用可能
                        ...
                        # 個別ファイルアップロード
                        for file_path in encrypted_folder.rglob('*'):
                            if file_path.is_file() and not any(pattern in str(file_path) for pattern in ["*.tmp", "*.temp", ".DS_Store", "Thumbs.db"]):
                                relative_path = file_path.relative_to(encrypted_folder)
                                upload_file(
                                    path_or_fileobj=str(file_path),
                                    path_in_repo=f"batch_encrypted/{username}/{relative_path}",
                                    repo_id=self.full_repo_name,
                                    repo_type="dataset",
                                    token=self.api.token
                                )
