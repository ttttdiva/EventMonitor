#!/usr/bin/env python3
"""
FANBOX作品取得・メディアダウンロード
gallery-dlを使用してFANBOXの作品メタデータとメディアを取得
Cookie認証（www.fanbox.cc_cookies.txt）を使用
"""

import sys
import json
import subprocess
import logging
import os
import shutil
import tempfile
import time
from datetime import datetime
from http.cookiejar import MozillaCookieJar
from pathlib import Path
from typing import List, Dict, Any, Optional, Tuple

import requests

from .path_utils import get_media_base_paths
from .subprocess_utils import run_with_idle_timeout


class FanboxExtractor:
    """gallery-dlを使用してFANBOX作品を取得"""

    def __init__(self, config: dict):
        self.config = config
        self.logger = logging.getLogger("EventMonitor.FanboxExtractor")
        self.fanbox_config = config.get('fanbox', {})

        # メディア保存先（一時）
        self.media_dir = Path(config.get('media', {}).get('save_dir', 'data/media')) / 'fanbox'

        # ラッパースクリプトのパス
        self.wrapper_path = Path(__file__).parent / 'gallery_dl_wrapper.py'

        # Cookieファイル
        self.cookie_file = Path('cookies/www.fanbox.cc_cookies.txt')

        # バッチサイズ
        batch_cfg = self.fanbox_config.get('max_batch_size', 50)
        try:
            self.batch_size = max(1, int(batch_cfg))
        except (TypeError, ValueError):
            self.batch_size = 50

        # アカウント到達性キャッシュ（サイクルごとにクリア）
        self._account_reachable: Dict[str, bool] = {}

    # ------------------------------------------------------------------
    # Public API
    # ------------------------------------------------------------------

    def resolve_display_name(self, creator_id: str) -> Optional[str]:
        """
        表示名を取得（軽量なFANBOX APIから抽出）

        Args:
            creator_id: クリエイターID（例: ashiyama）

        Returns:
            display name（取得できた場合）、取得失敗時はNone
        """
        display_name = self._resolve_display_name_via_api(creator_id)
        if display_name:
            self.logger.info(f"Resolved fanbox creator {creator_id} -> {display_name}")
            return display_name

        self.logger.warning(
            f"Display name not found via FANBOX API for creator {creator_id}"
        )
        return None

    def check_account_reachable(self, creator_id: str) -> bool:
        """軽量リチェック: gallery-dl --range 1-1 でアカウントの到達性を確認"""
        url = f"https://www.fanbox.cc/@{creator_id}"
        cmd = [
            sys.executable,
            str(self.wrapper_path),
            '--cookies', str(self.cookie_file),
            '-q', '-j',
            '--range', '1-1',
            url,
        ]

        try:
            result = subprocess.run(
                cmd,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                timeout=60,
            )

            if result.returncode == 0:
                return True

            stderr_lower = (result.stderr or "").lower()
            not_found_patterns = ["404", "not found", "no results", "does not exist"]
            if any(p in stderr_lower for p in not_found_patterns):
                self.logger.info(
                    f"FANBOX creator {creator_id} appears unreachable: "
                    f"{result.stderr[:200]}"
                )
                return False

            # 明確な削除シグナルなし → 一時的エラーとして到達可能扱い
            return True

        except subprocess.TimeoutExpired:
            return True  # タイムアウトは一時的
        except Exception as e:
            self.logger.error(
                f"Error checking fanbox reachability for {creator_id}: {e}"
            )
            return True  # エラーは一時的

    def clear_reachability_cache(self) -> None:
        """サイクル間のキャッシュをクリア"""
        self._account_reachable.clear()

    # ------------------------------------------------------------------
    # 新着チェック（軽量）
    # ------------------------------------------------------------------

    _FANBOX_LIST_API = "https://api.fanbox.cc/post.listCreator"
    _FANBOX_HEADERS = {
        "Accept": "application/json, text/plain, */*",
        "Origin": "https://www.fanbox.cc",
        "Referer": "https://www.fanbox.cc/",
        "User-Agent": (
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 (KHTML, like Gecko) "
            "Chrome/120.0.0.0 Safari/537.36"
        ),
    }

    def _load_fanbox_cookies(self) -> Optional[requests.cookies.RequestsCookieJar]:
        """Netscape形式のクッキーファイルを読み込んでrequests用に変換"""
        if not self.cookie_file.exists():
            self.logger.warning(f"FANBOX cookie file not found: {self.cookie_file}")
            return None
        try:
            jar = MozillaCookieJar(str(self.cookie_file))
            jar.load(ignore_discard=True, ignore_expires=True)
            rjar = requests.cookies.RequestsCookieJar()
            for c in jar:
                rjar.set_cookie(c)
            return rjar
        except Exception as e:
            self.logger.error(f"Failed to load fanbox cookies: {e}")
            return None

    def _resolve_display_name_via_api(self, creator_id: str) -> Optional[str]:
        """post.listCreator APIの最初の投稿からクリエイター表示名を取得する。"""
        cookies = self._load_fanbox_cookies()
        if cookies is None:
            return None

        url = f"{self._FANBOX_LIST_API}?creatorId={creator_id}&limit=1"
        try:
            resp = requests.get(
                url,
                headers=self._FANBOX_HEADERS,
                cookies=cookies,
                timeout=15,
            )
            if resp.status_code == 404:
                self._account_reachable[creator_id] = False
                return None
            if resp.status_code != 200:
                self.logger.warning(
                    f"FANBOX list API returned {resp.status_code} while resolving "
                    f"display name for {creator_id}"
                )
                return None

            body = resp.json().get("body") or {}
            return self._extract_display_name_from_list_body(body)
        except requests.RequestException as e:
            self.logger.warning(
                f"FANBOX list API request failed while resolving display name for "
                f"{creator_id}: {e}"
            )
            return None
        except Exception as e:
            self.logger.warning(
                f"Unexpected error resolving FANBOX display name for {creator_id}: {e}"
            )
            return None

    @staticmethod
    def _extract_display_name_from_list_body(body) -> Optional[str]:
        """post.listCreator APIレスポンスbodyから user.name を取り出す。"""
        if isinstance(body, list):
            items = body
        elif isinstance(body, dict):
            items = body.get("items") or []
        else:
            return None

        for item in items:
            if not isinstance(item, dict):
                continue
            user_data = item.get("user") or {}
            if not isinstance(user_data, dict):
                continue
            display_name = str(user_data.get("name") or "").strip()
            if display_name:
                return display_name
        return None

    def check_new_post_ids(
        self,
        creator_id: str,
        known_ids: set,
        max_pages: int = 10,
        page_sleep: float = 1.0,
    ) -> Optional[List[str]]:
        """
        FANBOXのpost.listCreator APIを直接叩いて新着post IDのみを抽出する軽量チェック。

        gallery-dlがitems()内で各post.info APIを叩く挙動を避けるため、
        一覧APIだけでページングしknown_idsとの重複を見つけた時点で停止する。

        Args:
            creator_id: クリエイターID
            known_ids: DBに既にあるpost IDの集合
            max_pages: 最大ページ数（暴走防止）
            page_sleep: ページ間スリープ秒

        Returns:
            新着post IDのリスト（新しい→古い順）。通信失敗時はNone。
        """
        cookies = self._load_fanbox_cookies()
        if cookies is None:
            return None

        known_str = {str(k) for k in known_ids}
        new_ids: List[str] = []
        url: Optional[str] = (
            f"{self._FANBOX_LIST_API}?creatorId={creator_id}&limit=10"
        )
        pages = 0
        hit_overlap = False

        try:
            session = requests.Session()
            while url and pages < max_pages:
                pages += 1
                resp = session.get(
                    url,
                    headers=self._FANBOX_HEADERS,
                    cookies=cookies,
                    timeout=30,
                )
                if resp.status_code == 404:
                    self.logger.info(
                        f"FANBOX creator {creator_id} not found (404)"
                    )
                    self._account_reachable[creator_id] = False
                    return []
                if resp.status_code != 200:
                    self.logger.warning(
                        f"FANBOX list API returned {resp.status_code} for "
                        f"{creator_id} (page {pages})"
                    )
                    return None

                body = resp.json().get("body") or {}
                if isinstance(body, list):
                    # 一部クリエイターでbodyがlistとして返るケースがある
                    items = body
                else:
                    items = body.get("items") or []
                for item in items:
                    pid = str(item.get("id") or "")
                    if not pid:
                        continue
                    if pid in known_str:
                        hit_overlap = True
                        continue
                    new_ids.append(pid)

                if hit_overlap:
                    self.logger.info(
                        f"FANBOX {creator_id}: hit known id at page {pages}, "
                        f"found {len(new_ids)} new posts"
                    )
                    self._account_reachable[creator_id] = True
                    return new_ids

                next_url = body.get("nextUrl") if isinstance(body, dict) else None
                if not next_url:
                    self.logger.info(
                        f"FANBOX {creator_id}: reached end of post list at "
                        f"page {pages}, found {len(new_ids)} new posts"
                    )
                    self._account_reachable[creator_id] = True
                    return new_ids

                url = next_url
                if page_sleep > 0:
                    time.sleep(page_sleep)

            self.logger.warning(
                f"FANBOX {creator_id}: reached max_pages={max_pages} without "
                f"overlap ({len(new_ids)} new ids so far) — 全件フェッチにフォールバック推奨"
            )
            return None

        except requests.RequestException as e:
            self.logger.error(
                f"FANBOX list API request failed for {creator_id}: {e}"
            )
            return None
        except Exception as e:
            self.logger.error(
                f"Unexpected error in check_new_post_ids for {creator_id}: {e}"
            )
            return None

    def fetch_user_works(
        self, creator_id: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        指定FANBOXクリエイターの作品メタデータを取得

        Args:
            creator_id: クリエイターID（例: ashiyama）
            limit: 取得件数制限

        Returns:
            作品情報のリスト
        """
        url = f"https://www.fanbox.cc/@{creator_id}"

        cmd = [
            sys.executable,
            str(self.wrapper_path),
            '--cookies', str(self.cookie_file),
            '-o', 'sleep-request=3',
            '-v',
            '-j',
        ]

        if limit:
            cmd.extend(['--range', f'1-{limit}'])

        cmd.append(url)

        self.logger.info(
            f"Fetching fanbox works for creator {creator_id} (limit: {limit or 'all'})"
        )

        try:
            result = run_with_idle_timeout(cmd, idle_timeout=120, rate_limit_retries=0)

            if result.stderr:
                self.logger.debug(
                    f"gallery-dl stderr for fanbox {creator_id}: "
                    f"{result.stderr[:500]}"
                )

            if result.returncode != 0:
                self.logger.error(
                    f"gallery-dl error for fanbox {creator_id}: "
                    f"returncode={result.returncode} stderr={result.stderr[:300]}"
                )
                stderr_lower = (result.stderr or "").lower()
                not_found_patterns = [
                    "404", "not found", "no results", "does not exist"
                ]
                if any(p in stderr_lower for p in not_found_patterns):
                    self._account_reachable[creator_id] = False
                return []

            output = result.stdout.strip()
            if not output:
                self.logger.info(
                    f"No output from gallery-dl for fanbox {creator_id}"
                )
                self._account_reachable[creator_id] = True
                return []

            self._account_reachable[creator_id] = True
            return self._parse_gallery_dl_output(output)

        except subprocess.TimeoutExpired:
            self.logger.error(
                f"Timeout fetching fanbox works for creator {creator_id}"
            )
            return []
        except Exception as e:
            self.logger.error(f"Error fetching fanbox works: {e}")
            return []

    def fetch_works_metadata_by_ids(
        self, creator_id: str, post_ids: List[str]
    ) -> List[Dict[str, Any]]:
        """
        特定の投稿IDのメタデータを個別に取得（メディア再チェック用）

        Args:
            creator_id: クリエイターID
            post_ids: 取得対象のpost_idリスト

        Returns:
            作品情報のリスト
        """
        if not post_ids:
            return []

        all_works: List[Dict[str, Any]] = []
        for post_id in post_ids:
            url = f"https://www.fanbox.cc/@{creator_id}/posts/{post_id}"
            cmd = [
                sys.executable,
                str(self.wrapper_path),
                '--cookies', str(self.cookie_file),
                '-v',
                '-j',
                url,
            ]

            try:
                result = run_with_idle_timeout(cmd, idle_timeout=60, rate_limit_retries=0)

                if result.returncode != 0:
                    self.logger.debug(
                        f"gallery-dl error for fanbox post {post_id}: "
                        f"returncode={result.returncode}"
                    )
                    continue

                output = result.stdout.strip()
                if not output:
                    continue

                works = self._parse_gallery_dl_output(output)
                all_works.extend(works)

            except Exception as e:
                self.logger.error(
                    f"Error fetching metadata for fanbox post {post_id}: {e}"
                )
                continue

        self.logger.info(
            f"Fetched metadata for {len(all_works)} of {len(post_ids)} "
            f"fanbox posts (media recheck)"
        )
        return all_works

    def download_media_for_works(
        self,
        creator_id: str,
        work_ids: List[str],
        move_to_images: bool = True,
    ) -> Dict[str, List[str]]:
        """
        特定の作品IDのメディアをダウンロード

        Args:
            creator_id: クリエイターID
            work_ids: ダウンロード対象のpost_idリスト
            move_to_images: imagesディレクトリに移動するか

        Returns:
            作品IDごとのメディアファイルパスの辞書
        """
        if not work_ids:
            return {}

        self.media_dir.mkdir(parents=True, exist_ok=True)

        # ダウンロード前のファイルを記録
        existing_files = set()
        if self.media_dir.exists():
            for f in self.media_dir.rglob('*'):
                if f.is_file():
                    existing_files.add(f)

        remaining = list(work_ids)
        all_media_paths: Dict[str, List[Path]] = {}
        all_downloaded: List[Path] = []
        consecutive_no_progress = 0
        max_no_progress = 5
        attempt = 0

        while remaining and consecutive_no_progress < max_no_progress:
            attempt += 1
            current_batch = remaining[:self.batch_size]

            self.logger.info(
                f"Attempt {attempt}: Downloading media for {len(current_batch)} of "
                f"{len(remaining)} remaining fanbox works"
            )

            url_file_path = None
            try:
                with tempfile.NamedTemporaryFile(
                    mode='w', suffix='.txt', delete=False
                ) as url_file:
                    for post_id in current_batch:
                        url_file.write(
                            f"https://www.fanbox.cc/@{creator_id}/posts/{post_id}\n"
                        )
                    url_file_path = url_file.name

                cmd = [
                    sys.executable,
                    str(self.wrapper_path),
                    '--cookies', str(self.cookie_file),
                    '-d', str(self.media_dir),
                    '-o', 'filename={id}_{num}.{extension}',
                    '-o', 'sleep-request=3',
                    '-v',
                    '--input-file', url_file_path,
                ]

                result = run_with_idle_timeout(cmd, idle_timeout=180, rate_limit_retries=0)

                if result.returncode != 0:
                    self.logger.warning(
                        f"gallery-dl issues: {result.stderr[:200]}"
                    )

                new_files_by_work = self._collect_downloaded_files(
                    current_batch, self.media_dir, existing_files
                )

                if new_files_by_work:
                    total_files = sum(
                        len(fs) for fs in new_files_by_work.values()
                    )
                    self.logger.info(
                        f"Downloaded {total_files} files for "
                        f"{len(new_files_by_work)} works"
                    )

                    all_media_paths.update(new_files_by_work)
                    for fs in new_files_by_work.values():
                        all_downloaded.extend(fs)

                    remaining = [
                        wid for wid in remaining
                        if wid not in new_files_by_work
                    ]
                    for fs in new_files_by_work.values():
                        existing_files.update(fs)

                    consecutive_no_progress = 0
                else:
                    self.logger.warning(f"No new files in attempt {attempt}")
                    consecutive_no_progress += 1

            except subprocess.TimeoutExpired:
                self.logger.warning(f"Timeout in attempt {attempt}")
                partial = self._collect_downloaded_files(
                    current_batch, self.media_dir, existing_files
                )
                if partial:
                    all_media_paths.update(partial)
                    for fs in partial.values():
                        all_downloaded.extend(fs)
                    remaining = [
                        wid for wid in remaining if wid not in partial
                    ]
                    for fs in partial.values():
                        existing_files.update(fs)
                    consecutive_no_progress = 0
                else:
                    consecutive_no_progress += 1

            except Exception as e:
                self.logger.error(f"Error in attempt {attempt}: {e}")
                consecutive_no_progress += 1

            finally:
                if url_file_path:
                    try:
                        os.unlink(url_file_path)
                    except Exception:
                        pass

        if remaining:
            self.logger.error(
                f"Failed to download media for {len(remaining)} fanbox works"
            )

        # ファイルをimages/videosディレクトリに移動
        dir_name = f"fanbox_{creator_id}"
        final_paths: Dict[str, List[str]] = {}
        if move_to_images and all_downloaded:
            moved = self._move_to_images_dir_with_mapping(
                all_downloaded, dir_name
            )
            for wid, orig_files in all_media_paths.items():
                paths = []
                for orig in orig_files:
                    if orig in moved:
                        abs_path = str(moved[orig]).replace('\\', '/')
                        if '/images/' in abs_path:
                            rel = 'images/' + abs_path.split('/images/')[1]
                        elif '/videos/' in abs_path:
                            rel = 'videos/' + abs_path.split('/videos/')[1]
                        else:
                            rel = abs_path
                        paths.append(rel)
                if paths:
                    final_paths[wid] = paths
        else:
            final_paths = {
                wid: [str(f) for f in fs]
                for wid, fs in all_media_paths.items()
            }

        self._cleanup_media_dir()
        return final_paths

    # ------------------------------------------------------------------
    # Internal helpers
    # ------------------------------------------------------------------

    def _parse_gallery_dl_output(
        self, output: str
    ) -> List[Dict[str, Any]]:
        """gallery-dlのJSON出力をパースして作品情報リストを返す"""
        works: Dict[str, Dict[str, Any]] = {}

        try:
            if not output.startswith('['):
                self.logger.error("Unexpected gallery-dl output format")
                return []

            all_items = json.loads(output)

            for item in all_items:
                if not isinstance(item, list) or len(item) < 2:
                    continue

                item_type = item[0]

                if item_type == 2 and isinstance(item[1], dict):
                    # Type 2: ディレクトリメタデータ [2, metadata_dict]
                    work_info = self._extract_work_info(item[1])
                    if work_info:
                        wid = work_info['id']
                        if wid not in works:
                            works[wid] = work_info

                elif (item_type == 3
                      and len(item) >= 3
                      and isinstance(item[2], dict)):
                    # Type 3: ファイルダウンロード [3, media_url, metadata_dict]
                    media_url = item[1]
                    media_meta = item[2]
                    post_id = str(media_meta.get('id', ''))
                    if post_id:
                        if post_id not in works:
                            work_info = self._extract_work_info(media_meta)
                            if work_info:
                                works[post_id] = work_info
                        if post_id in works and media_url not in works[post_id]['media']:
                            works[post_id]['media'].append(media_url)

        except json.JSONDecodeError as e:
            self.logger.error(
                f"Failed to parse gallery-dl fanbox output: {e}"
            )
            return []

        result = list(works.values())
        self.logger.info(f"Parsed {len(result)} fanbox works")
        return result

    def _extract_work_info(
        self, data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """gallery-dlのデータから作品情報を抽出"""
        try:
            post_id = data.get('id')
            if not post_id:
                return None

            post_id = str(post_id)

            # 日付パース（publishedDatetimeを優先）
            date_str = data.get('publishedDatetime') or data.get('date', '')
            if date_str:
                if isinstance(date_str, str):
                    for fmt in (
                        "%Y-%m-%dT%H:%M:%S%z",
                        "%Y-%m-%dT%H:%M:%S",
                        "%Y-%m-%d %H:%M:%S",
                        "%a, %d %b %Y %H:%M:%S %z",
                    ):
                        try:
                            dt = datetime.strptime(date_str, fmt)
                            break
                        except ValueError:
                            continue
                    else:
                        dt = datetime.now()
                elif isinstance(date_str, (int, float)):
                    dt = datetime.fromtimestamp(date_str)
                else:
                    dt = datetime.now()
                date_iso = dt.isoformat() if dt.tzinfo else dt.isoformat() + 'Z'
            else:
                date_iso = datetime.now().isoformat() + 'Z'

            # ユーザー情報
            user_data = data.get('user', {})
            display_name = user_data.get('name', '') if isinstance(user_data, dict) else ''
            creator_id = data.get('creatorId', '')

            # タグ
            tags = data.get('tags', [])
            if isinstance(tags, list):
                tags = [t if isinstance(t, str) else str(t) for t in tags if t]
            else:
                tags = []

            # センシティブ判定（hasAdultContent == True）
            sensitive = bool(data.get('hasAdultContent', False))

            return {
                'id': post_id,
                'display_name': display_name,
                'text': data.get('title', ''),
                'date': date_iso,
                'url': f"https://www.fanbox.cc/@{creator_id}/posts/{post_id}",
                'media': [],  # Type 3エントリで追加される
                'tags': tags,
                'sensitive': sensitive,
                'creator_id': creator_id,
                'fee_required': data.get('feeRequired', 0),
                'source': 'fanbox',
                'platform': 'fanbox',
            }

        except Exception as e:
            self.logger.error(f"Error extracting fanbox work info: {e}")
            return None

    def _collect_downloaded_files(
        self,
        work_ids: List[str],
        output_dir: Path,
        existing_files: set,
    ) -> Dict[str, List[Path]]:
        """ダウンロード済みファイルを作品IDごとに収集"""
        new_files_by_work: Dict[str, List[Path]] = {}
        if not output_dir.exists():
            return new_files_by_work

        for f in output_dir.rglob('*'):
            if not f.is_file() or f in existing_files:
                continue

            filename = f.name
            # ファイル名パターン: {id}_{num}.ext
            for post_id in work_ids:
                if (filename.startswith(f"{post_id}_")
                        or filename.startswith(f"{post_id}.")):
                    if post_id not in new_files_by_work:
                        new_files_by_work[post_id] = []
                    new_files_by_work[post_id].append(f)
                    break

        # ファイル名順でソート
        for wid in new_files_by_work:
            new_files_by_work[wid].sort(key=lambda f: f.name)

        return new_files_by_work

    def _move_to_images_dir_with_mapping(
        self, files: List[Path], dir_name: str
    ) -> Dict[Path, Path]:
        """ダウンロードしたファイルをimages/videosディレクトリに移動"""
        mapping: Dict[Path, Path] = {}
        try:
            images_base, videos_base = get_media_base_paths(self.config)

            images_dir = images_base / dir_name
            videos_dir = videos_base / dir_name
            images_dir.mkdir(parents=True, exist_ok=True)
            videos_dir.mkdir(parents=True, exist_ok=True)

            video_extensions = {
                '.mp4', '.mov', '.avi', '.webm', '.mkv', '.flv', '.wmv',
                '.m4v', '.mpg', '.mpeg', '.3gp', '.3g2', '.ts', '.vob',
                '.ogv', '.f4v', '.asf', '.rm', '.rmvb', '.m2ts', '.mts',
                '.m3u8', '.m3u',
                '.gif', '.gifv',
                '.mp3', '.m4a', '.wav', '.flac', '.aac', '.ogg', '.opus',
                '.wma', '.aiff', '.alac', '.oga',
            }

            for src_file in files:
                filename = src_file.name
                is_video = src_file.suffix.lower() in video_extensions

                if is_video:
                    dest_file = videos_dir / filename
                else:
                    dest_file = images_dir / filename

                if dest_file.exists():
                    mapping[src_file] = dest_file
                else:
                    shutil.copy2(src_file, dest_file)
                    mapping[src_file] = dest_file

            self.logger.info(
                f"Moved {len(mapping)} fanbox media files for {dir_name}"
            )

        except Exception as e:
            self.logger.error(f"Failed to move fanbox files: {e}")

        return mapping

    def _cleanup_media_dir(self):
        """一時メディアディレクトリを削除"""
        try:
            if self.media_dir.exists():
                shutil.rmtree(self.media_dir)
        except Exception as e:
            self.logger.error(f"Failed to cleanup media dir: {e}")
