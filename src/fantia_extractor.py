#!/usr/bin/env python3
"""
Fantia作品取得・メディアダウンロード
gallery-dlを使用してFantia.jpの作品メタデータとメディアを取得
Cookie認証（_session_id）を使用
"""
...
class FantiaExtractor:
    """gallery-dlを使用してFantia作品を取得"""

    def __init__(self, config: dict):
        self.config = config
        self.logger = logging.getLogger("EventMonitor.FantiaExtractor")
        self.fantia_config = config.get('fantia', {})

        # メディア保存先（一時）
        self.media_dir = Path(config.get('media', {}).get('save_dir', 'data/media')) / 'fantia'

        # ラッパースクリプトのパス
        self.wrapper_path = Path(__file__).parent / 'gallery_dl_wrapper.py'

        # Cookieファイル
        self.cookie_file = Path('cookies/fantia.jp_cookies.txt')

        # バッチサイズ
        batch_cfg = self.fantia_config.get('max_batch_size', 50)
        try:
    ...
    def resolve_display_name(self, fanclub_id: str) -> Optional[str]:
        """
        ...
        """
        works = self.fetch_user_works(fanclub_id, limit=1)
        if not works:
            self.logger.warning(
                f"No works found for fantia fanclub {fanclub_id}, cannot resolve display name"
            )
            return None

        display_name = works[0].get('display_name', '')
        if display_name:
            self.logger.info(f"Resolved fantia fanclub {fanclub_id} -> {display_name}")
            return display_name

        self.logger.warning(
            f"Display name not found in metadata for fantia fanclub {fanclub_id}"
        )
        return None

    def check_account_reachable(self, fanclub_id: str) -> bool:
        """軽量リチェック: gallery-dl --range 1-1 でアカウントの到達性を確認"""
        url = f"https://fantia.jp/fanclubs/{fanclub_id}"
        cmd = [
            sys.executable,
        ...
        try:
            ...
            if any(p in stderr_lower for p in not_found_patterns):
                self.logger.info(
                    f"Fantia fanclub {fanclub_id} appears unreachable: "
                    f"{result.stderr[:200]}"
                )
                return False
            ...
            self.logger.error(
                f"Error checking fantia reachability for {fanclub_id}: {e}"
            )
            return True  # エラーは一時的
    ...
    def fetch_user_works(
        self, fanclub_id: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        指定Fantiaファンクラブの作品メタデータを取得
        ...
        self.logger.info(
            f"Fetching fantia works for fanclub {fanclub_id} (limit: {limit or 'all'})"
        )
        ...
        try:
            ...
            if result.stderr:
                self.logger.debug(
                    f"gallery-dl stderr for fantia {fanclub_id}: "
                    f"{result.stderr[:500]}"
                )

            if result.returncode != 0:
                self.logger.error(
                    f"gallery-dl error for fantia {fanclub_id}: "
                ...
                if any(p in stderr_lower for p in not_found_patterns):
                    self._account_reachable[fanclub_id] = False
                return []

            output = result.stdout.strip()
            if not output:
                self.logger.info(
                    f"No output from gallery-dl for fantia {fanclub_id}"
                )
                self._account_reachable[fanclub_id] = True
                return []
            ...
            self.logger.error(
                f"Timeout fetching fantia works for fanclub {fanclub_id}"
            )
            return []
        except Exception as e:
            self.logger.error(f"Error fetching fantia works: {e}")
            return []

    def fetch_works_metadata_by_ids(
        self, fanclub_id: str, post_ids: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for post_id in post_ids:
            ...
            try:
                ...
                if result.returncode != 0:
                    self.logger.debug(
                        f"gallery-dl error for fantia post {post_id}: "
                ...
                self.logger.error(
                    f"Error fetching metadata for fantia post {post_id}: {e}"
                )
                continue

        self.logger.info(
            f"Fetched metadata for {len(all_works)} of {len(post_ids)} "
            f"fantia posts (media recheck)"
        )
        return all_works
    ...
    def download_media_for_works(
        ...
        while remaining and consecutive_no_progress < max_no_progress:
            ...
            self.logger.info(
                ...
                f"{len(remaining)} remaining fantia works"
            )
        ...
        if remaining:
            self.logger.error(
                f"Failed to download media for {len(remaining)} fantia works"
            )

        # ファイルをimages/videosディレクトリに移動
        dir_name = f"fantia_{fanclub_id}"
    ...
    def _parse_gallery_dl_output(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to parse gallery-dl fantia output: {e}"
            )
            return []

        result = list(works.values())
        self.logger.info(f"Parsed {len(result)} fantia works")
        return result
    ...
    def _extract_work_info(
        ...
        try:
            ...
            # 日付パース（posted_atを優先）
            date_str = data.get('posted_at') or data.get('date', '')
            if date_str:
            ...
            # ユーザー情報（"NULL"文字列はAPI上のプレースホルダなのでスキップ）
            fanclub_user_name = data.get('fanclub_user_name', '')
            if fanclub_user_name and fanclub_user_name.upper() == 'NULL':
                fanclub_user_name = ''
            display_name = (
                fanclub_user_name
                or data.get('fanclub_name', '')
            )
            ...
            # センシティブ判定（rating == "adult"）
            rating = str(data.get('rating', '')).lower()
            sensitive = (rating == 'adult')

            return {
                'id': post_id,
                'display_name': display_name,
                'text': data.get('post_title', ''),
                'date': date_iso,
                'url': f"https://fantia.jp/posts/{post_id}",
                ...
                'fanclub_id': str(data.get('fanclub_id', '')),
                'source': 'fantia',
                'platform': 'fantia',
            }

        except Exception as e:
            self.logger.error(f"Error extracting fantia work info: {e}")
            return None
    ...
    def _move_to_images_dir_with_mapping(
        ...
        try:
            ...
            self.logger.info(
                f"Moved {len(mapping)} fantia media files for {dir_name}"
            )

        except Exception as e:
            self.logger.error(f"Failed to move fantia files: {e}")
