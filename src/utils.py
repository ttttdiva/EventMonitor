...
def setup_logging(log_level: str = "INFO", log_dir: Path = None) -> logging.Logger:
    ...
    # ログファイル名に日時を含めて実行ごとに生成(app_YYYYMMDD_HHMMSS.log)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = log_dir / f"app_{timestamp}.log"

    # 古いログファイルを削除（最新5個を保持）
    existing_logs = sorted(log_dir.glob("app_*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
    for old_log in existing_logs[5:]:
        try:
            old_log.unlink()
        except OSError:
            pass
